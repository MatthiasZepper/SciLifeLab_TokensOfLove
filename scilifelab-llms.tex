...
\begin{frame}{Analogy of the model training process: Regression}
	\begin{columns}[T,onlytextwidth]
		...
		\column{0.5\textwidth}
		\begin{itemize}
			\item The model's formula corresponds to the \feature{architecture} of a LLM:
			\begin{description}
		...
		\end{itemize}
		\remark{A real LLM architecture is very different from these simple examples and has billions of parameters.}
		\column{0.5\textwidth}	
		\begin{figure}
			\includegraphics[width=\textwidth]{figures/NonlinearRegression.png}
			\caption{Example of a "training" a model}
		\end{figure}
